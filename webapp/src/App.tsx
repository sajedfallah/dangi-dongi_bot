...
export default function App() {
  ...
  async function createGroup(name:string){
    ...
    const list=await api.groups(user.id)
    setGroups(list)
  ...
  if (selectedGroup) {
    return <><GroupShell group={selectedGroup} view={groupView} onView={setGroupView} onBack={() => setSelectedGroup(null)}>
      ...
      {groupView === 'expense' && user && <ExpenseForm group={selectedGroup} user={user} members={members} onSaved={async()=>{setExpenses(await api.expenses(selectedGroup.id));setReport(await api.expenseReport(selectedGroup.id,user.id));setDebts(await api.debtReport(selectedGroup.id,user.id));setGroupView('overview')}}/>}
    </GroupShell>{creatingGroup&&<CreateGroupSheet onClose={()=>setCreatingGroup(false)} onCreate={createGroup}/>}</>
  }
...
function GroupOverview({members,expenses,report,debts,onView}:{members:Member[];expenses:Expense[];report:any;debts:any;onView:(v:GroupView)=>void}){
  ...
  return <><section className="group-hero soft-card"><div><span>{members.length} نفر</span><strong>{money(total)} تومان</strong><small>کل هزینه‌ها</small></div><div className="hero-photo">🏝️</div></section><button className="primary-neu wide" onClick={()=>onView('expense')}><Plus/> ثبت هزینه جدید</button><div className="action-grid"><Action icon={<ReceiptText/>} text="هزینه‌ها"/><Action icon={<WalletCards/>} text="تسویه‌ها" onClick={()=>onView('settlements')}/><Action icon={<Users/>} text="اعضا" onClick={()=>onView('members')}/><Action icon={<PieChart/>} text="گزارش‌ها" onClick={()=>onView('reports')}/><Action icon={<Settings/>} text="دسته‌بندی‌ها"/><Action icon={<Users/>} text="دعوت عضو"/></div><SectionTitle title="آخرین هزینه‌ها" action={`${expenses.length} مورد`}/><div className="stack">{expenses.slice(0,5).map(x=><div className="expense-row soft-card" key={x.id}><div className="expense-icon">{categoryLabels[x.category||'other']?.split(' ')[0]||'🧾'}</div><div><b>{x.title}</b><small>{categoryLabels[x.category||'other'] || x.category || 'سایر'}</small></div><strong>{money(x.amount)}<small> تومان</small></strong></div>)}</div>{!expenses.length&&<div className="empty-state soft-card"><b>اولین هزینه را ثبت کن</b><p>هنوز هزینه‌ای در این حساب نیست.</p></div>}{debts?.transfers?.length===0&&expenses.length>0&&<div className="success-note soft-card">✅ این حساب در وضعیت تسویه است.</div>}</>
}
